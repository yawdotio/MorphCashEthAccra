    ...
    <div id="how-it-works" className="py-24 bg-base-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-base-content mb-4">How Morph Cash Works</h2>
          <p className="text-xl text-base-content/70">Get started in just a few simple steps</p>
        </div>
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
          {steps.map((step, index) => (
            <div key={step.step} className="relative text-center group">
              <div className="flex justify-center mb-6">
                <div className="w-16 h-16 bg-gradient-to-r from-primary to-secondary rounded-full flex items-center justify-center text-primary-content font-bold text-xl shadow-lg group-hover:scale-110 transition-transform duration-200">
                  {step.step}
                </div>
              </div>
              ...
              {index < steps.length - 1 && (
                <div className="hidden lg:block absolute top-8 right-0 transform translate-x-1/2 w-full">
                  <div className="h-px bg-gradient-to-r from-primary/30 to-secondary/30 w-full" />
                </div>
              )}
            </div>
          ))}
