  ...
  return (
    <div className="min-h-screen bg-base-200">
      ...
      <div className={`fixed inset-0 flex z-40 md:hidden ${sidebarOpen ? "" : "pointer-events-none"}`}>
        <div
          className={`fixed inset-0 bg-base-content/50 transition-opacity ${sidebarOpen ? "opacity-100" : "opacity-0"}`}
          onClick={() => setSidebarOpen(false)}
        />
        <div
          className={`relative flex-1 flex flex-col max-w-xs w-full bg-base-100 transition-transform ${
        ...
        >
          ...
          <div className="flex-1 h-0 pt-5 pb-4 overflow-y-auto">
            <div className="flex-shrink-0 flex items-center px-4 mb-8">
              ...
              <span className="font-bold text-xl text-base-content">MorphCash</span>
            </div>
            <nav className="mt-5 px-2 space-y-1">
              {navigation.map(item => {
                ...
                return (
                  ...
                  >
                    <item.icon
                      className={`${isActive ? "text-primary" : "text-base-content/50 group-hover:text-base-content/70"} mr-3 flex-shrink-0 h-6 w-6`}
                    />
                    {item.name}
                  </Link>
                );
          ...
          <div className="flex-shrink-0 flex border-t border-base-300 p-4">
            <div className="flex items-center">
              {user?.ensAvatar ? (
                <img src={user.ensAvatar} alt="User avatar" className="w-8 h-8 rounded-full" />
              ) : (
      ...
      <div className="hidden md:flex md:w-64 md:flex-col md:fixed md:inset-y-0">
        <div className="flex-1 flex flex-col min-h-0 border-r border-base-300 bg-base-100">
          ...
          <div className="flex-shrink-0 flex border-t border-base-300 p-4">
            <div className="flex items-center justify-between w-full">
              ...
              <button
                onClick={logout}
                className="text-base-content/50 hover:text-base-content/70 transition-colors duration-200"
                title="Logout"
              >
      ...
      <div className="md:pl-64 flex flex-col flex-1">
        <div className="sticky top-0 z-10 md:hidden pl-1 pt-1 sm:pl-3 sm:pt-3 bg-base-200">
          <button
            type="button"
            className="-ml-0.5 -mt-0.5 h-12 w-12 inline-flex items-center justify-center rounded-md text-base-content/70 hover:text-base-content focus:outline-none focus:ring-2 focus:ring-inset focus:ring-primary"
